...
//        //更新玩家伤害吸收显示GUI
//        float f = total - v;
//        if (f > 0.0F && f < 3.4028235E37F && damageSrc.getTrueSource() instanceof ServerPlayer) {
//            ((ServerPlayer) damageSrc.getTrueSource()).addStat(Stats.DAMAGE_DEALT_ABSORBED, Math.round(f * 10.0F));
//        }
//        //结算伤害与伤害吸收
//        if (v != 0.0F) {
...
//                MobAttrSaveData saveData = MobAttrSaveData.getInstance((ServerLevel) livingEntity.world);
...
//                break;
//            case "arrow":
//                arrowDamage(source.getTrueSource(), (AbstractArrow) source.getImmediateSource(), amount, damage);
//                break;
//            case "sweetBerryBush":
//                damage.setP(2.0F);
//                break;
...
//     * 计算玩家是否暴击
//     */
//    public static boolean isCri(Player player) {
...
//            mobLv = MobAttributesUtils.getMobLv((ServerLevel) mob.world, mob.getUniqueID().toString());
//        }
...
//     * 计算箭矢伤害
//     */
//    private static void arrowDamage(Entity source, AbstractArrow arrow, float amount, Damage damage) {
//        if (source instanceof Player) {
//            damage.setP(Math.max((float) arrow.getDamage(), 4.0F));
//            ItemStack stack = ((Player) source).getItemStackFromSlot(EquipmentSlotType.MAINHAND);
//            if (!stack.isEmpty() && stack.getItem() instanceof AbstractRangedWeapon) {
//                //增加TEC
//                ((AbstractRangedWeapon) stack.getItem()).incrTec(stack);
//                //增加武技学习进度
//                ((Weapon) stack.getItem()).incrWeaponSkill(source.getStringUUID());
//            }
...
//            lv = StoryPlayerDataManager.getLv(StoryPlayerDataManager.get(playerEntity.getStringUUID()).getXp());
//        } else if (entity instanceof MobEntity) {
//            if (!entity.world.isRemote()) {
//                lv = MobAttributesUtils.getMobLv((ServerLevel) entity.world, entity.getUniqueID().toString());
//            }
//        }
//        if (lv > 0) {
//            amount += lv * offset;
...
////                //最低1点,最高2(8/4)点
////                ((Player) livingEntity).inventory.func_234563_a_(source, Math.min(damage, 8.0F));
////            }
//            //计算伤害
