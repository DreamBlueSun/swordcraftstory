...
public class WeaponIntensifyBlock extends Block {

    public WeaponIntensifyBlock() {
    ...
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new WeaponIntensifyTileEntity();
    }
    ...
    public void onReplaced(BlockState state, World worldIn, BlockPos pos, BlockState newState, boolean isMoving) {
        if (!worldIn.isRemote && worldIn.getGameRules().getBoolean(GameRules.DO_TILE_DROPS)) {
            ...
            list.add(ItemRegistry.WEAPON_INTENSIFY_BLOCK.get().getDefaultInstance());
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote || handIn != Hand.MAIN_HAND) {
            ...
            if (!stack.isEmpty() && stack.getItem() instanceof Hammer) {
                Networking.INSTANCE.sendToServer(new SendPack("smithery.weaponIntensify", pos));
            }
        }
        return ActionResultType.SUCCESS;
