...
public class ItemCollapseBlock extends CraftBlock {
    ...
    public MenuProvider getMenuProvider(@NotNull BlockState state, @NotNull Level level, @NotNull BlockPos pos) {
        return new SimpleMenuProvider((openContainerId, inventory, player) ->
                new ItemCollapseMenu(openContainerId, inventory, ContainerLevelAccess.create(level, pos)),
                new TranslatableComponent("解体锻冶台"));
    }
    ...
    public void spawnAfterBreak(@NotNull BlockState state, @NotNull ServerLevel level, @NotNull BlockPos pos, @NotNull ItemStack stack) {
        Containers.dropItemStack(level, pos.getX(), pos.getY(), pos.getZ(), ItemRegistry.ITEM_COLLAPSE_BLOCK.get().getDefaultInstance());
