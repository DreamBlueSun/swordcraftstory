...
import com.marisa.swordcraftstory.block.craft.menu.AbstractItemCollapseMenu;
...
@OnlyIn(Dist.CLIENT)
public class ItemCollapseCombinerScreen<T extends AbstractItemCollapseMenu> extends AbstractContainerScreen<T> implements ContainerListener {
    private final ResourceLocation menuResource;

    public ItemCollapseCombinerScreen(T p_98901_, Inventory inventory, Component component, ResourceLocation location) {
    ...
    protected void renderBg(PoseStack stack, float p_98918_, int p_98919_, int p_98920_) {
        ...
        if (this.menu.getSlot(0).hasItem() && !this.menu.getSlot(1).hasItem()) {
            this.blit(stack, i + 75, j + 45, this.imageWidth, 0, 28, 21);
        }
