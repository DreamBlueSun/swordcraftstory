...
public class SlanderOre extends AbstractOre {
    ...
    public ItemStack weaponMake(ItemStack mouldStack) {
        ...
        switch (((Mould) mouldStack.getItem()).type()) {
            case BOW:
                stack = ItemRegistry.THIN_BOW.get().getDefaultInstance();
                break;
            case SWORD:
                stack = ItemRegistry.THIN_SWORD.get().getDefaultInstance();
                break;
            case AXE:
                stack = ItemRegistry.THIN_AXE.get().getDefaultInstance();
                break;
            default:
    ...
    @Override
    public Item asSword() {
        return ItemRegistry.THIN_SWORD.get();
    }

    @Override
    public Item asBow() {
        return ItemRegistry.THIN_BOW.get();
    }

    @Override
    public Item asAxe() {
        return ItemRegistry.THIN_AXE.get();
    }
    ...
    public IntensifyAttr getIntensifyAttr() {
        return new IntensifyAttr(Intensifys.SLANDER_ORE.getId(), Intensifys.SLANDER_ORE.getShow(), 0, 5, 0, 0);
    }

    @Override
    public boolean build(ItemStack stack) {
        stack.getOrCreateTag().put("story_combat_model_change", IntTag.valueOf(WeaponModels.SLANDER_ORE.getId()));
