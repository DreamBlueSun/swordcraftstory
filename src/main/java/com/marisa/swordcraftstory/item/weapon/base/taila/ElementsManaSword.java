...
public class ElementsManaSword extends SwordItem implements WeaponModel {

    public ElementsManaSword() {
        super(new Tier() {
            @Override
            public int getUses() {
                return 2021;
    ...
    public boolean build(ItemStack stack) {
        switch (((Weapon)stack.getItem()).type()) {
            case SWORD:
                stack.getOrCreateTag().put("story_combat_model_change", IntTag.valueOf(WeaponModels.ELEMENTS_MANA_SWORD.getId()));
                return true;
            case BOW:
            case AXE:
            default:
