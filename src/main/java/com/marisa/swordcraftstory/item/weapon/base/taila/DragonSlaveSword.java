...
public class DragonSlaveSword extends SwordItem implements WeaponModel {
    ...
    public DragonSlaveSword() {
        ...
        }, 10, -2.4F, new Item.Properties().tab(StoryGroup.MODEL_CHANGE_GROUP));
    }
    ...
    public boolean build(ItemStack stack) {
        switch (((Weapon)stack.getItem()).type()) {
            case SWORD:
                stack.getOrCreateTag().put("story_combat_model_change", IntTag.valueOf(WeaponModels.DRAGON_SLAVE_SWORD.getId()));
                return true;
            case BOW:
            case AXE:
            default:
